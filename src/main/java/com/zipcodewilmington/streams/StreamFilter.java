...
public class StreamFilter {
    ...
    public StreamFilter() { this(Stream.generate(new PersonFactory()::createRandomPerson).limit(100), RandomUtils.createCharacter('A', 'Z'));
    }
    ...
    public List<Person> toListMultiLine() {
        ...
        List<Person> personList= toListOneLine();
        Stream<Person> pStream = personList.stream();
        pStream= pStream.filter(person -> (person.getName().startsWith(this.startingCharacter)));
    ...
    public Person[] toArrayMultiLine() {
        ...
        Person[] array= toArrayOneLine();
        Stream<Person> pStream= Arrays.asList(array).stream();
        pStream= pStream.filter(person -> person.getName().startsWith(this.startingCharacter));

        return pStream.toArray(Person[]::new);
    }
